...
import azkaban.constants.LogProperties;
...
public class Emailer extends AbstractMailer implements Alerter {
    ...
    public void sendErrorEmail(ExecutableFlow flow, String... extraReasons) {
        ...
        String logName = "_job" + LogProperties.FILE_SEP + flow.getExecutionId() + LogProperties.FILE_SEP + flow.getFlowId
                () + LogProperties.LOG_SUFFIX;
        try {
            File logFile = FileIOUtils.findFiles(flow.getExecutionPath(), logName);
            if (logFile != null) {
                message.addAttachment(logFile);
            } else {
                logger.info("can not find log file...");
            }
        } catch (MessagingException e) {
            e.printStackTrace();
            logger.error("add log file failed...");
        }
